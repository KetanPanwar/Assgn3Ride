...
#         if(resp_send.status_code == 200):
        if (usr in d):
            ...
            fata["query"] = "insert"
            resp_send = requests.post(
                "http://52.203.103.216:80/api/v1/db/write", json=fata)
            print("recieved")
            return jsonify({}), 201
        else:
...
def details_of_rides(rideId):
    ...
    if request.method == 'GET':
        ...
        resp_send = requests.post("http://34.194.124.13:80/api/v1/db/read", json=usr)
        resp_send1 = requests.post(
            "http://52.203.103.216:80/api/v1/db/read", json=data1)
        if(resp_send.status_code == 200 and resp_send1.status_code == 200):
            ...
            if usr["username"] not in d["users"]:
                resp_send = requests.post(
                    "http://52.203.103.216:80/api/v1/db/write", json=od)
        ...
        if resp_send1.status_code == 400:
            ...
            resp_send = requests.delete(
                "http://52.203.103.216:80/api/v1/db/write", json=data)
